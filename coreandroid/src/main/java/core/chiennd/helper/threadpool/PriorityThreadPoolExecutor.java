package core.chiennd.helper.threadpool;
...
public class PriorityThreadPoolExecutor extends ThreadPoolExecutor {
    ...
    public Future<?> submit(Runnable task) {
        ...
        if (task == null || !(task instanceof PriorityRunnable)) {
            priorityRunnable = new PriorityRunnable(Priority.LOW);
        } else {
            priorityRunnable = (PriorityRunnable) task;
        }
        PriorityFutureTask futureTask = new PriorityFutureTask(priorityRunnable);
        execute(futureTask);
        return futureTask;
    }
